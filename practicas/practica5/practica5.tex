...
\end{figure}
\noindent \\\\Haremos una prueba que nos debería devolver un resultado negativo ya que no tenemos almacenados partidos con empate:\\
\begin{figure}[hbt!]
    \centering
...
\end{figure}
\noindent \\\\No hay duda... este Web Controller es totalmente operativo.
\newpage
...
carpeta \textit{report} (que está relacionada con la generación de documentos impresos en Odoo) y un botón dentro de la vista de partidos para que
...
\end{figure}
\noindent \\La creación de la plantilla es problemática entre \textbf{Odoo 18} y la librería \textbf{wkhtmltopdf}, ya que
...
Por las dificultades de crear una plantilla decente me he visto obligado a generar gran parte del código del template con asistencia de IA.\\
...
\end{figure}
\noindent \\\\En la siguiente página los enlaces:
\newpage
\begin{figure}[hbt!]
    \centering
...
\end{figure}
\noindent \\Tenemos todo lo necesario para hacer pruebas. Como bien sabemos, toca reiniciar contenedores y actualizar módulos dentro de Odoo.\\\\
...
\end{figure}
\noindent \\Comprobemos si funciona:\\
\begin{figure}[hbt!]
    \centering
...
\end{itemize}
\noindent \\Si no recibe ninguna de las órdenes anteriores deberá devolver la respuesta ``\textit{Orden no soportada}''. Como toque personal me 
permito concatenar ``\textit{ Sólo se admite `Crear', `Modificar', `Consultar' y `Borrar' }''.\\\\
Lo primero que haremos será descargarnos las librerías necesarias en la máquina real. Estas serán \textit{`pip install requests python-telegram-bot python-dotenv'}.\\\\
Una vez tengamos correctamente configurado nuestro Python crearemos nuestra aplicación en el archivo \textit{BotTelegram.py}. A continuación las capturas del código:
\newpage
\noindent \\
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1\textwidth]{61.png}
    \caption{Cargamos archivo .env, definimos constantes y creamos la función que hará las peticiones. POST y PUT irán en el cuerpo, GET y DELETE en la propira URL}
\end{figure}
\newpage
\noindent \\
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1\textwidth]{62.png}
    \caption{Creamos función que manejará los mensajes recibidos desde Telegram donde se filtrarán y procesarán adecuadamente. En la última parte del código creamos el objeto
    que iniciará la aplicación}
\end{figure}
\noindent \\Una vez creada la aplicación (\textbf{\underline{esto no es un módulo de Odoo, es una aplicación propia}}) sólo nos falta crear el bot en Telegram. \\\\
En las siguientes páginas se mostrarán las capturas del proceso de creación:
\newpage
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.6\textwidth]{61.jpg}
    \caption{Tras haber comenzado una conversación con BotFather, clicamos en el botón \textit{`Crear Nuevo Bot'}}
\end{figure}
\newpage
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.6\textwidth]{62.jpg}
    \caption{Rellenamos el formulario a nuestro gusto y clicamos en el botón \textit{`Crear Nuevo Bot'}}
\end{figure}
\newpage
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.6\textwidth]{63.jpg}
    \caption{Una vez creado, copiamos el token generado y nos lo llevamos a nuesto proyecto (preferiblemente como variable de entorno)}
\end{figure}
\newpage
\noindent Sólo queda probar que todo funciona correctamente (contenedor Odoo levantado y aplicación iniciada):
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.6\textwidth]{63.png}
    \caption{Mandamos peticiones POST desde Telegram}
\end{figure}
\newpage
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.6\textwidth]{64.png}
    \caption{Ahora peticiones PUT, GET Y DELETE}
\end{figure}
\newpage
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1\textwidth]{65.png}
    \caption{Desde la terminal comprobamos que todo concuerda}
\end{figure}
\noindent \\\\Las pruebas han sido satisfactorias. \\\\
Me resulta curioso la forma en la que está diseñado el controlador del API-REST-SOCIOS, ya que permite gestionar los métodos POST y PUT por peticiones en url y body,
y sin embargo los métodos GET y DELETE sólo permite peticiones por url, lo cual ha sido confuso al empezar a crear la aplicación.
\newpage
...
\end{figure}
\noindent \\De forma muy similar al código de ejemplo anterior, importamos las librerías necesarias (en este caso las librerías clave son
